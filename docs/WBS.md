...
- [~] 2.23 *(core shipped, session 19 first act)* **The smith paints roads; the engine infers the segments** *(Daniil, playtest 9; session 19)*. The segment-only brushes (playtest 5) cannot author a junction. **T-junctions become first-class 3-port cell types (4 orientations)** — the dev's first proposal ("T is just `R`") was wrong and Daniil caught it: `R` carries all four ports, so two adjacent `R`-built structures merge, rolling back touch-without-merge exactly where roads are dense. A true 4-way *intersection* (crossing AND merging) legitimately has four ports and stays `R`; crossing WITHOUT merging is the bridge cell (4.9), a type carrying two independent port pairs. Authoring: **painting is the primary mode** — mouse drag paints road cells and each road cell's type is re-inferred from the painted neighbourhood (2 in line → straight, corner → bend, 3 → T, 4 → intersection), with per-change undo; segment brushes stay as manual override. SHIPPED: pure-neighbourhood inference (mask -> shape table incl. T/3/U/E and 'R' at 4), edge-centre virtual continuation creating entries, terrain-paint erases and survivors re-shape; segment buttons removed (were the patch pile - the default 'R' brush with no button was playtest 10's 'borked engine'). Remaining: drag-painting, per-change undo, bridge cells at 4.9.
