...
- [ ] 2.35 *(since PR #123 the Core column is ground: the two cells touching the face exist and are buildable; the boons are still one shared Loadbearing multiplier — defaults per tower proposed in HANDOVER for session 26)* **Cells next to the Core are precious** *(Daniil, 2026-09-05; PRD §4.5 — design intent, own session)*. With the Core a face at the east edge, at most four buildable cells touch it. The rules and the towers will make those cells the most valuable ground on the map: **every tower gets a unique boon when placed next to the Core**, not one shared multiplier. Replaces the Loadbearing relic's flat triple range once designed. Exact details are a design conversation before the new towers (25).
...
- [~] 6.9 *(session 25, PRs #125–#126: `sequences` (charge, fire, cool, hit) with per-frame `ms` in the sprite format; `attackLook()` plays them on the fire clock, or a placeholder derived from the idle frame; **interpolation NOT built** on Daniil's "more animation frames first" — his call)* **Animation engine, second pass** *(Daniil, 2026-09-05; PRD §13)*: enemies and projectiles interpolated between ticks (20 Hz sim, 60 Hz picture); idle cycles of many frames; **per-tower attack animations** — charge, shot, cooldown — as named sequences in the sprite format keyed to `Sim.events`, authored in the sprite editor. The frame model from 4.1 grows sequences; nothing in the sim changes.
