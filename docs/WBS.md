...
- [!] 2.23 **Tile Smith: explicit segment brushes in a matrix** *(Daniil, playtests 9-11; REVERSED after the inference attempt failed)*. **Inference is abandoned.** The dev proposed neighbourhood-inference over Daniil's gesture idea and shipped it; it is structurally incapable of the one thing the road model exists for: **explicit segment types encode PORTS, while inference derives ports from adjacency - so any two adjacent lane-A cells necessarily merge.** Touch-without-merge (`|` beside `|`: no E/W ports, touching and separate) is unrepresentable by ANY adjacency rule. Daniil raised this at pushback time; the dev did not test the proposal against the requirement. It also guesses rather than obeys, which is the opposite of an authoring tool's job.
  **The answer**: a **button per segment type, laid out as a MATRIX** (not one line) - straights `-` `|`; bends `L` `J` `F` `7`; Ts `T` `U` `E` `3`; crossroads `R`; lane-B `r`; terrain `G` `K` `O` `C`. Clunkier and **deterministic**: what you click is what you place, which is what makes touching-not-merging authorable at all. Grouped rows so 16 brushes stay findable; drag-paint with the held brush; per-change undo.
- [ ] 2.26 **Validity: no roads to nowhere** *(Daniil, playtest 11)*. The entry-point rule (2.20) admits a **dead-end spur**: a stub hanging off a through-road reaches an entry *via that road*, so it passes while visibly leading nowhere - the tile in Daniil's screenshot was declared valid. Tighten to: **every road cell must lie on a route between two distinct entries** (or between an entry and the Core). Verify against every shipped tile before adopting, as with 2.20 - `twin_bend` is the fixture that punishes a careless formulation.
