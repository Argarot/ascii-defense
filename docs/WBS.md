...
brush→2.23 · actual sprites on the palette→2.23 · "make sure the validity
...
- [x] 2.23 *(session 19)* **Tile Smith: explicit segment brushes in a matrix** *(Daniil, playtests 9-11; REVERSED after the inference attempt failed)*. **Inference is abandoned and deleted.** The dev proposed neighbourhood-inference over Daniil's gesture idea and shipped it; it is structurally incapable of the one thing the road model exists for: **explicit segment types encode PORTS, while inference derives ports from adjacency - so any two adjacent lane-A cells necessarily merge.** Touch-without-merge (`|` beside `|`: no E/W ports, touching and separate) is unrepresentable by ANY adjacency rule. Daniil raised this at pushback time; the dev did not test the proposal against the requirement. It also guesses rather than obeys, which is the opposite of an authoring tool's job.
  **Shipped as Daniil's 4×4 matrix** (from his mock review, 2026-08-18): rows `F T 7 |` / `E X 3 -` / `L U J B` / `G R O C` — the road block composes into a box figure with straights and the bridge in the fourth column, terrain beneath; two gutter labels ("roads", "terrain"), no per-type descriptions. Brushes are the **actual sprites** (a GL palette through the shared `drawTerrainCell`), each showing its ports as road stubs into the gutter; hover names the brush (X and B share a sprite by design). **Ground is the default brush**, drag-paint with the held brush, per-change undo (button + Ctrl+Z). **Cell nomenclature migrated in the same change** (Daniil): crossroads `R`→`X`, rock `K`→`R`, lane-B `r`→`B` *bridge* — reserving the letter for the true 4.9 crossing cell whose semantics land later; all assets crawled (engine, library, tilegen, schema, view, tests), and the localStorage minted pool letter-migrates v1→v2 on load, because under the new alphabet an old road `R` would silently reinterpret as rock. Golden hash unmoved through the rename.
