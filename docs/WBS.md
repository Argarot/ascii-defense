...
- [~] 5.7 *(session 28, PR 2: rarity with teeth - a base rarity on every relic, a wave-weighted roll on every draw, rare and epic tiers with their own numbers and text, tags and set effects at two and three of a tag, rarity frames on slots and cards. Replace, remove and combine follow in PR 3)* **Rarity with power, and relics you can replace, remove and combine** *(Daniil, 2026-09-05; PRD §7.6, §7.8)*: rare means stronger; a held relic can be dropped or swapped; combining makes §7.6's fusion concrete (recipes as content).
