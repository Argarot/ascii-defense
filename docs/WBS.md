...
cannot mint junctions → **2.23 (paint-inference primary; T/X are `R` cells, no
new types; caught by Daniil — dev validated against the library, not the
authoring surface)** · rotation identity → 2.24 *(selection already rotates;
...
- [ ] 2.23 **The smith paints roads; the engine infers the segments** *(Daniil, playtest 9; session 19)*. The segment-only brushes (playtest 5) cannot author a junction — while the engine and the shipped library already support them via the omni `R` cell (3+ neighbours = a T or a cross; `ROAD_PORTS.R` is all four sides). Caught by Daniil, not the dev; the dev had validated the entry-point rule against the *library* (where tilegen's junctions masked the gap) instead of against the *authoring surface*. Fix: **painting is the primary mode** — the author paints road cells and every road cell's type is re-inferred from its painted neighbours (2 in line → `-`/`|`, corner → bend, 3 → `R`, 4 → `R`). Inference from the **painted set**, not mouse-gesture direction (dev's refinement of Daniil's proposal): order-independent, idempotent, undo-friendly, and it re-derives correctly when cells are erased. Segment brushes stay as a secondary manual-override mode. **Bridges**: no inference contortion — when 4.9 lands, a dedicated bridge cell type carries two independent port pairs (N-S over E-W), and painting a stroke across an existing perpendicular road offers it explicitly. No new cell types before that: T and X are `R`, by construction.
- [ ] 2.24 **Rotation-canonical tiles** *(Daniil, playtest 9; session 19)*. A tile and its 90° rotations are ONE tile. The generator's index already resolves and deals all four rotations of every library tile (mapgen.ts, `for rotation of [0,1,2,3]`), so selection needs nothing; what is missing is **dedup at the pool level**: a canonical form (lexicographically smallest of the four rotations), tilegen collapsing its four orientation families into canonical tiles (24 generated → ~6), the minted pool storing canonical form so the same shape drawn sideways is not a second asset, and the index skipping rotation-duplicate signatures so one shape is not weighted four times.
- [ ] 4.26 **A blast reads as a blast** *(Daniil, playtest 9; session 18 triage)*. The 2.19 flash disc shrank the visual (0.6·r); Daniil's intent: the KILL radius is the flash, and a **shockwave** travels visibly beyond it — flash disc = full r (the area that kills, exactly), debris/ring continues past r fading (cosmetic, clearly weaker). Truth and drama are both served: nothing inside the flash survives the lie test, nothing outside it dies.
- [ ] 2.25 **Health pips carry colour** *(Daniil, playtest 9; session 18 triage)*. Two braille dots cannot carry a health bar alone; the pip's colour ramps green → amber → red with hp01 so the glyph and the colour together read at a glance (invariant 10 untouched: colour presents, nothing branches on it).
