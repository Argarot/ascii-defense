...
**Round 33 (session 33, Daniil's ten items of 2026-09-08 "lets do these first + the stuff you proposed"):** indexed by HIS numbering → 1 Esc in the workshop and the purse #195 (4.41; the tier's source named when the purse lacks it) · 2 no relic offer in a whole run #195 (4.41; the clock deals the owed offer, a call still carries the debt - item 18 kept) · 3 how-to-play as cards in the game #196 (4.42; encounter cards on every first meeting) · 4 the title's towers animate #195 (4.41) · 5 the tree drawn as a tree #199 (4.45; PRD §22) · 6 chests too frequent #195 (4.41; thirty seconds) · 7 boss chests with sprites and rarity #198 (4.44; PRD §4.9) · 8 the Codex as the wiki of what was met #196 (4.42; PRD §20) · 9 the mender's green field #195 (4.41) · 10 the creative page #197 (4.43; PRD §21). The plan's five: the generator's hundred → every legal routing shape (nine) plus sixty land tiles #200 (4.46; PRD §23 - the honest count) · the multiset #201 (4.47; PRD §24) · previews at 1× #202 (4.48; PRD §25) · the stranger test protocol #203 (4.49; **not yet run** - his hands) · the art brief for the seven #203 (4.50). Not built: a hundred ROAD shapes (the law allows nine); the stranger test itself; Grim and relics against the seven; the register's remaining rows.

**Round 32 (session 32, "go" on the HANDOVER plan Enemies II, 2026-09-08):** the plan taken as the contract → PR 1 the seven bodies #190 (4.37; PRD §9.1b) · PR 2 packs and formations #191 (4.38; PRD §9.2; `countMax` as bodies, from the register) · PR 3 counter legibility #192 (4.39) · PRs 4 and 5 in one: the balance pass and the enemy sweep #193 (4.40; docs/lab/enemy-sweep-2026-09-08.md; Standard 6 + 5 a wave at ×1.07, Grim ×1.09). His defaults taken: the seven's names and looks from the generator; the ramp's target as the reference dying between 16 and 24 on Standard (it lands at 22-24), Calm unchanged. Not built: the art agent's studies for the seven (placeholders stand); `countMax` counting a pack's members dropped at the slot cap; Grim and relics against the seven are not read.

**Round 31 (session 31, "make this session big... you have all night", 2026-09-07 night into 2026-09-08):** his asks, in his order - (a) a tutorial for new players, the expected sequence highlighted in a pulsing yellow box → PR 1 #181 (4.30; PRD §20) with the fork step added in PR 6 #186; (b) the first rebalancing pass for the early game of players without the features the engine expects → PR 2 #182 (4.31; the armour floor, the boss multiplier by weight, Calm's own curve; docs/lab/base-sweep-2026-09-07.md) and the live-board addendum in PR 6 (placement and forks, not the curve); (c) tech debt → PR 4 #184 (4.32, the keyboard on every page), PR 7 #187 (4.35, applicable relics - the register's "offers weighted by applicability"), personal bests in PR 8 #188; (d) bugs → PR 5 #185 (4.33, a refused spawn waits, the wave ceiling), the logic comb PR 8 #188 (4.36: Thick Walls, the refund of what was paid, dead duplicate picks, aura reach, the offer's debt, Frostbite under a freeze, Overflow's type, the hash's holes); (e) the fine-tooth comb over the logic and the UI → PR 3 #183 (comb one), PR 6 #186 (comb two: the page sweep), PR 8 (comb three). Not built: the register's multiset of tile copies, terminals once per session and the 2× tile previews (each a refactor, not a comb item); `countMax` counting bodies rather than queue entries; the dead `fireRateMul`/`rangeAdd` seams (schema-live, content unused - left with a note in PRD §7.6).

**Round 30 (session 30, "go build session 30", 2026-09-07):** the HANDOVER plan taken as the contract → PR 1 the menu language and the workshop drawn as a tree #175 (7.10.1; item 30's first half; the plan's PRs 1 and 2 folded) · PR 2 the Tile Smith as a page #176 (7.10.2; 7.5's first half; his "fold tile smith into the main menu") · PR 3 the relic plate and the relic surfaces #177 (7.10.3; 9.14, 9.19) · PR 4 chests by rarity, blasts by tower, beams by path #178 (7.10.4; 9.13, 9.25, 9.27) · PR 5 the tree sweep with a loadout #179 (7.10.5). Not built: full keyboard operation; the art agent's 6×5 relic sprites and the chest sprite (the view draws both until then); the pricing of Smith-minted tiles against shipped prices is one function and his eye.

**Round 29 (session 29, "go for session 29", 2026-09-06 night into 2026-09-07):** the HANDOVER plan taken as the contract with his six answers (a tree with branches · only Ore, in tiers - three · about five runs to the towers · unlocks in the run code · the Tile Smith after every tile is bought) → PR 0 the fix bundle #167 (9.7-9.12, 9.18, 9.24; the Bastion's plus shipped as FOUR, his "a plus" literally) · PR 1 the tree as content and identity #168 (7.9.1; item 2 forging caps, item 3 wins earn relics) · PR 2 the workshop #169 (7.9.2; locked Threats, endless 9.22) · PR 4 Ore by tier #170 (7.9.3, 9.4) · PR 5 the tile shop and the Smith's door #171 (7.9.4, 7.7 first half) · PR 6 the lab at tree states and the pricing #172 (7.9.5) · PR 7 the codex with locked entries, undiscovered fusions, legendary #173 (7.9.6, 7.9.7; 9.1, 9.15, 9.23). The plan's PR 3 ("the base set wired") landed inside PR 1. Not built: copies of a tile as a multiset (the generator places a chosen id once), personal bests, the Tile Smith as a page, the menus rework.
...
- [x] 1.7.2 *(session 28: thirty-six relics plus five fusion-only (PR 4); a full row asks which relic a pick replaces and any offer can be skipped (PR 3) - the layer never switches itself off)* **The pool runs dry silently** — 11 relics, caches deal duplicates, `unheldPool()` empties, `maybeOffer` returns early and the acquisition layer switches off with no message. Short term: more relics + an honest "pool exhausted" state. Real fix is 2.7.
...
- [x] 2.7 *(session 28, PRs 2-4: rarity with teeth and a wave-weighted roll; salvage for Ore by rarity; combine two of a kind or a recipe pair; twenty more relics on nineteen new engine knobs with rare/epic tiers; placeholder icons for the twenty-five without art; Foundry shipped from the PROPOSED table)* **Relic economy** (PRD §7.6): pool grown well beyond what one run can drain; **fusion** (several relics into one stronger) and **salvage** (trade back for Ore); a much larger share single-use; rarity weighting (D5). This is what makes more slots safe to add rather than trivialising.
...
- [x] 4.49 *(session 33, PR #203; docs/STRANGER-TEST.md)* **The stranger test protocol** (the plan's PR 4): ten scored moments, the before and after, the round filed in this index; the gate for row 33 is a 2 on NEXT, the answer and wave 5. **Not yet run** - Daniil's or his friend's hands.
- [x] 4.50 *(session 33, PR #203; docs/ART-AGENT.md §2)* **The art brief for the seven** (the plan's PR 5): the fourteen bodies with their sizes, the marks the view draws that a study may own, the chests.
...
- [x] 4.44 *(session 33, PR #198; PRD §4.9)* **Boss chests** (feedback item 7): a boss leaves a crowned chest where it falls, rarer with the wave, a minute to claim, paid through the boss table at its rarity; `chest` and `chest_boss` 4×3 sprites from the generator, blinking with the window; the validator knows the chest's size.
- [x] 4.43 *(session 33, PR #197; PRD §21)* **The creative page** (feedback item 10): purse, board, spawn any kind, grant any relic at any rarity, the tree's nodes toggled in the meta save; behind `?dev` or Ctrl+Shift+D; unrecorded verbs, the page says so.
- [x] 4.42 *(session 33, PR #196; PRD §20)* **Encounter cards and the Codex** (feedback items 3 and 8): a card pops over the board on the first meeting of an enemy kind, a tower kind, a chest, or boon ground - once ever, in the meta save; the title's CODEX with a BOONS section and `???` pages for enemies not yet met.
- [x] 4.41 *(session 33, PR #195)* **Feedback 2026-09-08, the fix bundle** (items 1, 2, 4, 6, 9): Esc leaves the workshop and the history; the purse in the workshop's bottom band and the tier's source named when the purse lacks it; an owed offer dealt by the clock at the latest; the title's towers idle in their frames; chests every thirty seconds; the mender's green pulse.
- [x] 4.40 *(session 32, PR #193; docs/lab/enemy-sweep-2026-09-08.md; PRD §9)* **The balance pass and the enemy sweep** (the plan's PRs 4 and 5 in one): six curve variants read against the target; Standard 6 + 5 a wave at ×1.07, Grim ×1.09; the base and tree sweeps re-baselined; `--enemies` - every body alone against every tower line, the counter table with its reading; the live board played once.
- [x] 4.39 *(session 32, PR #192; PRD §9.1b)* **Counter legibility**: marks on the body (a mound, a cross, braces, a trail, a plank on the facing side), words and marks per trait in the strip with a formations line under NEXT, the answer line in the column, the tutorial's NEXT step.
- [x] 4.38 *(session 32, PR #191; PRD §9.2)* **Packs and formations**: a wave composes from packs (one kind, one front, one formation) until its count of bodies is filled; column, wedge (from wave 4), wall (from wave 8); the boss on a beat behind the escort; the queue entry carries def, spacing and front; the preview names the packs; `countMax` counts bodies.
- [x] 4.37 *(session 32, PR #190; PRD §9.1b)* **Enemies II, the bodies**: seven more in the roster (courser, ram, blob, mender, mole, pavise, the Warden), seven trait rules in `traits.ts` each with a test (sprint, charge, split, heal, burrow, frontshield, bulwark), `splitInto` and `bossOnly` in the schema, placeholder sprites, the codex entries.
- [x] 4.36 *(session 31, PR #188; PRD §7.6 "the logic comb")* **The logic comb and comb pass three**: an audit of the sim against the cards - Thick Walls symmetric on salvage, refunds of what was paid, no dead duplicate picks or held fusion targets, the readier cooldown survives a forge, aura relics widen the plus, an empty pool keeps the offer's debt, Frostbite counts a freeze, Overflow carries the hit's type; the hash covers what decided a hit; honest cards (Wide Net, Superheated, Salvage Rights' epic, the support sets reachable). Personal bests on the history page; the strip's kind column fits "swarmling"; the G key is "tile seams" everywhere; the tutorial toggle says what a click does.
- [x] 4.35 *(session 31, PR #187)* **Applicable relics** (the register's "relic offers weighted by applicability"): `needsTower` on four relics; a run without the tower leaves the relic out of its pool (worker and lab); the codex names the tower; nested mods no longer print as "[object Object]".
- [x] 4.34 *(session 31, PR #186)* **Comb pass two**: the live Calm probe (three plain Bolts far from the Core die at wave 8; eight forked ones by the Core hold fifteen with no breach) - the tutorial's ground step names the Core's ground and a new fork step (13 steps) ends on the first upgrade; a page sweep - body lines and the next-wave composition wrap at words (never "shellb"), a note never draws left of the plate, the threat's phrase is a body line, the title's settings show no run keys, "1 special tile", names for the generator's tiles, the codex wraps to the screen; Calm's win summary says Ore is what Calm banks; debug ops choose/sell/stats/tutorial.
- [x] 4.33 *(session 31, PR #185)* **The sim's edges**: a body the slot cap refuses waits in the queue (it was dropped - a free wave); a wave holds at most sixty bodies (`countMax`) so endless stays a ramp of hp, not of spawn time.
- [x] 4.32 *(session 31, PR #184)* **The keyboard on every page** (4.24's other half): the arrows walk a page's rows and tiles, Enter clicks the one they are on, the cursor row is lit; in a run the keys were already whole.
...
- [x] 5.7 *(session 28, PR 3: twelve relic slots; full slots ask which held relic a pick replaces; skip any offer; the held relic's card - salvage for Ore by rarity, combine two of a kind into the next rarity or a recipe pair into a fusion-only relic (five duos); the slot pulses when its rule fires, the summary counts fires. PR 2: rarity with teeth - a base rarity on every relic, a wave-weighted roll on every draw, rare and epic tiers with their own numbers and text, tags and set effects at two and three of a tag, rarity frames on slots and cards. Replace, remove and combine follow in PR 3)* **Rarity with power, and relics you can replace, remove and combine** *(Daniil, 2026-09-05; PRD §7.6, §7.8)*: rare means stronger; a held relic can be dropped or swapped; combining makes §7.6's fusion concrete (recipes as content).
- ~~5.8 **The passive layer**~~ *(built in session 28 PR 1 as a separate layer; Daniil, 2026-09-06 evening: "this is not what I wanted" - folded back into the relic pool the same day as eleven tower-mod relics with tiers; D26 reversed)*
...
- [x] 7.7 **The tile shop** (PRD §11.1, resolves D9): special tiles bought with meta-currency; the pool becomes a **multiset of owned copies** rather than a set, so generation may place at most what you own; tier-N ore nodes purchased with tier-(N-1) ore. Appearance likelihood is a calibration knob, not a constant. *(One copy each shipped in session 29, PR #171 - 7.9.4; the multiset with the carve placing every copy in session 33, PR #201 - 4.47.)*
