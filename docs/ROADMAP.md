...
| 20 | **The backbone reassessment** *(NEXT — Daniil's call at playtest 16: "drifting into pile-of-patches territory"; patching stopped by agreement)* | WBS 2.27, in order: **(a)** the generator SPECIFICATION — Daniil's six rules plus the deal-phase guarantees — as invariants checked in one place, with a constraint-first rebuild where the nine-pass pipeline cannot satisfy them cleanly; **(b)** the worker lifecycle (init/resume/save/genError) as an explicit state machine; **(c)** deterministic run/map identity design (URL seeds rejected — modifiers always exist); **(d)** playtest 16's three bugs (boon-on-void, dropped bridge specials, phantom resume after quit) written as regression tests against the spec BEFORE rebuilding. Opens with the spec conversation, not code | Spec committed; every invariant check and all three regression tests green; **Daniil generates and plays loadout-heavy runs without producing a defect list** — his verdict |
