...
| 32 | **Enemies II** *(the enemy half of the old 28–29 content completeness; deferred 2026-09-06 evening as "bells and whistles while the game itself is not finished", then again by session 31's tutorial mandate - so it is NEXT; the PR list is HANDOVER "Next session, proposed")* | Seven new bodies across the trait matrix with new traits (splitter, healer, burrower, charger, shieldbearer, a second boss); the wave composer's packs and formations; counter legibility on the body and the strip; the Laser and ramp balance pass against a stated target; the enemy sweep | Wave 10 looks different from wave 5 for what walks in; every new body says what it does by standing there; no build clears every wave alone |
| 33 | **Content completeness, the rest** *(the tile half of the old 28–29, plus row 28's remainder; the Tile Smith in the shell landed in session 30, full keyboard operation in session 31)* | Tile pool to ~100; the stranger test | Two runs do not resemble each other; a stranger plays unaided |
| 34 | **Calibration I** *(was 28, then 31)* | WBS 1.5.1/1.5.2 + 3.1–3.4: bot policy, `calibrate`/`check`, human offset from Daniil's replays, `balance.yml` gate, seed-corpus sweeps | Injected regression caught; no trivial or unwinnable seed in ≥500 runs |
| 34–35 | **Meta progression, full** *(was 29–30, then 31–32; plus 7.8, the monetization door — D27; the tree, run history with bests and the in-game Tile Smith landed in sessions 29–31)* | Tech tree stages 1–2 beyond the shipped tree, dailies, replay sharing, **the tile-loadout slot economy** (7.5) | Finishing a run visibly changes the next one |
| 36 | **Calibration II** *(was 31, then 34)* | WBS 3.6: recalibrate with the meta layer live — tech-tree multipliers, pool unlocks and chosen tile loadouts all move player power underneath the curves calibration I fixed. Re-baseline `balance.yml`, re-sweep the seed corpus at several tree states | No trivial or unwinnable seed at any tech-tree state the player can actually hold |
| 36–38 | **Presentation at scale** *(was 32–34, then 34–36)* | Full art pass with per-upgrade tower identity (4.11), effects for every attack shape (6.3), enemy trait markers, UI art (4.13), **6.7 relic art at board-glyph scale**, **6.8 smoothness via spatial phase**, biomes, minimal SFX. The art round-trip proof (6.1) opens this block | The board reads as a place, not a diagram |
| 39–40 | **Beta hardening + release** *(was 35–36, then 37–38)* | Perf and bundle budgets in CI, browser matrix, error-path audit, save migration tests, external playtest with strangers, release process | PRD §17 stable beta |

**40 sessions to stable beta** (→ 40 on 2026-09-08: session 31 was bought
by Daniil's friend's playtest - a tutorial and a comb over the early game
took the row Enemies II held, and the ledger's later rows move by two).
The arc before that: 36 → 32 by merging themes that had been
