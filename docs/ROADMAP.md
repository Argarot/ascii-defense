...
| 19 | **The smith, done properly — then map agency** *(NEXT; re-scoped by playtests 9–11)* | **2.23 first and alone until Daniil says it works**: a button per segment type in a MATRIX (straights, bends, four Ts, crossroads, lane-B, terrain) — deterministic placement, because inference cannot express touch-without-merge and that is the model's whole point. Drag-paint, per-change undo. Then **2.26** tighten validity so a road cell must lie on a route between two entries (kills the dead-end spurs that currently pass). Then **2.24** rotation-canonical pools (dedup; the generator already deals all four rotations). Then **2.21** basics infinite / specials finite with guaranteed placement + the visual loadout picker, and **6.6** the shoreline if the day allows | **Daniil authors a tile with two roads that touch and do not merge, and one with a T-junction, without fighting the tool** — his verdict, not a passing test. Only then does the rest of the row start |
