...
| 29 | **The meta tree** *(Daniil, 2026-09-06 evening: "it's time to start working on the meta upgrade tree"; NEXT — the PR list is written after the discussion he asked for; the PRD's shape stands in HANDOVER "Next session, proposed")* | **PR 0 first: the fix bundle of the 2026-09-06 thought dump** (WBS 9.7–9.12, 9.18, 9.24). Then PRD §11 stage 1 and §11.1: the workshop page, banked Ore by tier as the currency, the relic pool and the tile pool as the sinks, run history, meta identity in the run code; **legendary rarity, tiers unlocked by forging, relics earned by wins, endless mode as a node, the codex with locked entries** (9.1, 9.2, 9.3, 9.15, 9.22, 9.23); one Refinery for every ore tier (9.4) | Finishing a run visibly changes the next one; five runs reach all towers; the codex shows what is still to find |
| 30 | **The Tile Smith in the shell** *(Daniil, 2026-09-06 evening: "fold tile smith into the main menu and make it part of the game")* | The Tile Smith as a page of the shell, opened from the workshop; features price the tile (§11.1); **the menus reworked to Stone-Story quality** (9.30) with the presentation items 9.14, 9.19, 9.25, 9.27 | Every essential piece of the game is in place; what remains is tuning; the shell reads as a product |
