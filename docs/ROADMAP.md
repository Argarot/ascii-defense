...
| 23 | **Toward alpha I — the board and the screen** *(opened 2026-09-05; PRs #111–#114 so far)* | Retune 1 (#111); the four playtest fixes — junction cells drawn by effective ports, the range as three thin rings, the Bolt's own shot, the ramp a notch down (#112); the art tooling tracked (#113); the thought dump sorted into the docs (#114). Then, in order: **2.30 the board fills** (specials first, the tree to ~90% coverage, every dead end an entrance, lanes balanced; the mapgen probe as a permanent sweep; D28 amendments first), difficulty re-measured on the new boards with a mixed-build lab, **4.27 the bottom strip** (wave composition with specialties, actives, build buttons with the towers' sprites), **4.28 the full-screen shell and the screen system**, **4.29 boon ground colours**, **2.32 the sprite contrast lint** | **Daniil's playtest**: five-special loadouts generate every time on his screen; junctions right; the bottom strip in use; a Standard run with an ordinary build reaches wave 10+ without the shortest lane deciding it |
| 24 | **Motion** | **6.9 animation engine, second pass** — interpolated movement, many-frame idles, per-tower attack animations (charge, shot, cooldown) keyed to sim events, authored in the sprite editor; **6.10 ability graphics** (the orbital laser as a beam from the top of the screen); 6.7 relic sprites at board scale. Sprites arrive from Daniil's art agent through the editor; the format grows sequences, the sim does not change | The board moves smoothly at 1× and 4× with nothing Daniil can point at as choppy; a Bolt visibly charges, fires and cools |
| 25 | **Combat identity** *(was 23)* | Opens with **D8**, the printing-trade lexicon — a conversation, and it comes first because everything below inherits the names. Then 2.8 damage types with real resistances; **2.31 statuses visible, every effect source tracked, every rule printed** (Frost + Concussive slows; Splinter's second blast); 4.10 attack shapes (chain, beam along a run of road, arc/wedge); **2.34 tower facing**; **5.1 towers to 8+** (tesla arcs, the rotating laser, short-range area, support, missile battery); **5.6 single-use nukes**; **D25 multi-cell towers decided** before any of them is built | A lexicon Daniil likes **before** the content that would inherit the old one; then **no single tower type clears a wave** and each new tower answers a wave the others cannot — proven by the sweep; Daniil can say which tower is for which enemy without a tooltip |
| 26 | **Relics II** *(absorbs the old 24)* | **D26 passives vs relics** (far more passive slots, not a doctrine copy) → 5.8; rarity with real power (5.7, the second half of D5); **replace, remove, combine** (5.7); 2.7 + 1.7.2 the pool grown past one run's drain, salvage, an honest "pool exhausted"; **2.22 loot tables and void chests** as the one mechanism every reward uses | Full slots is a decision, not a wall; the lab bounds the relic-driven power spread; every reward comes from one table mechanism |
| 27 | **First contact** *(was 25)* | WBS 4.23 onboarding prompts and how-to-play, 4.24 colourblind palette, full keyboard operation, HUD text scale | **A stranger plays unaided** — then actually hand it to one, before the content push their feedback should shape |
| 28–29 | **Content completeness** *(was 26–27)* | Towers to 8 with distinct attack shapes; enemies to ~14 across the trait matrix; relics to ~40 with fusion recipes; tile pool to ~100 | Two runs do not resemble each other — the count is whatever that takes, not a quota |
| 30 | **Calibration I** *(was 28)* | WBS 1.5.1/1.5.2 + 3.1–3.4: bot policy, `calibrate`/`check`, human offset from Daniil's replays, `balance.yml` gate, seed-corpus sweeps | Injected regression caught; no trivial or unwinnable seed in ≥500 runs |
