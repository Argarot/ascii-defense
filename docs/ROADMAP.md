...
| 26 | **Polish and the other menus** *(Daniil, 2026-09-05; NEXT — the PR list, gate, his part and the risk are in HANDOVER "Next session, proposed")* | His playtest list first; the how-to as the in-game codex generated from the same source as the catalogue; settings that exist and persist (text scale, colourblind palette 4.24, keys); the summary as the run's story; onboarding prompts (4.23); the lab's three missing instruments (adjacent and aimed placement, a per-wave-type reading) and the eight-tower sweep rerun; the Tile Smith as a page if there is room | Every page reachable from the title and back with no dead end; the how-to shows every tower, enemy and relic from content; settings survive a reload; the first run explains itself; his list closed |
| 27 | **Relics II** *(absorbs the old 24)* | **D26 passives vs relics** (far more passive slots, not a doctrine copy) → 5.8; rarity with real power (5.7, the second half of D5); **replace, remove, combine** (5.7); 2.7 + 1.7.2 the pool grown past one run's drain, salvage, an honest "pool exhausted"; **2.22 loot tables and void chests** as the one mechanism every reward uses | Full slots is a decision, not a wall; the lab bounds the relic-driven power spread; every reward comes from one table mechanism |
| 28 | **First contact** *(was 25)* | WBS 4.23 onboarding prompts and how-to-play, 4.24 colourblind palette, full keyboard operation, HUD text scale | **A stranger plays unaided** — then actually hand it to one, before the content push their feedback should shape |
