...
| D56 | **The long-turn special is re-minted out of straight runs** — **MINTED 2026-09-19, by default, unanswered** (call #381, his round of 2026-09-18 evening, item 1b; same deadline, same plan). His screenshot did not survive the chat and three things could be meant; the default is the tile: **Jogged Elbow (`gen_ne_1`) is re-minted as a taller turn with no touching segments** (`GGF-7 / GG|G| / GG|GL / GG|GG / GG|GG`), because on the BOARD its 2x2 block of touching road paints as one paved square and reads as a junction. Specials are never rolled, so no seed, band or golden hash moves. Drawing touching segments apart on the board — which would fix Double Step, Hump and Loop too — is filed as scope, not built. Reversible: one content edit |  | closed |
