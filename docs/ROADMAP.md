...
| 13 | **Scarcity + the run ends** *(proceed even if L dominates — crude-yet-functional now, exact tuning at calibration; Daniil is currently at wave 100 on an unchanged build)* | Finite ore deposits with richness, **ore-tier shape reserved** (D9 resolved — tiered cells and per-tier costs; no generator driver needed, since richer tiers are *tiles you buy*, M7), 10× slower mining, deposit readout; final wave + victory (D6), boss/elite waves. Numbers from the lab | A run **ends** — you win at wave N or die trying. Coasting with 5 towers dies |
