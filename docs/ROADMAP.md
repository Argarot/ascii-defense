...
| 13 | **Scarcity + the run ends** | Finite ore deposits with richness, **ore-tier mechanics reserved** (D9 — tiered cells, per-tier costs, a distribution driver; tier 1 only active), 10× slower mining, deposit readout; final wave + victory (D6), boss/elite waves. Numbers from the lab | A run **ends** — you win at wave N or die trying. Coasting with 5 towers dies |
