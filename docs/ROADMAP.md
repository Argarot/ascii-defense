...
| 18 | **The shell — screens + persistence** *(NEXT; was 22+23)* | **Opens with the Web Worker (D7)** — the pre-approved cut from 17, and it must land before the screens multiply. Then WBS 4.15–4.22: screen stack (generalising the offer modal), title menu, run setup, pause overlay, run summary; meta save, run save as seed + input log, schema versioning, export/import, settings screen — which is where session 16's reduced-motion flag gets its switch | A run starts from a menu and ends on a summary screen; close the tab mid-run, come back, continue; a corrupt save says so |
