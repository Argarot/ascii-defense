...
| 31 | **Enemies II** *(the enemy half of the old 28–29 content completeness; deferred 2026-09-06 evening as "bells and whistles while the game itself is not finished" — the game is finished in its pieces since session 30, so it is NEXT; the PR list is HANDOVER "Next session, proposed")* | Seven new bodies across the trait matrix with new traits (splitter, healer, burrower, charger, shieldbearer, a second boss); the wave composer's packs and formations; counter legibility on the body and the strip; the Laser and ramp balance pass against a stated target; the enemy sweep | Wave 10 looks different from wave 5 for what walks in; every new body says what it does by standing there; no build clears every wave alone |
