...
| ~~29~~ | **The meta tree** — DONE 2026-09-07 (session 29, PRs #167–#173): the fix bundle; the tree as content (24 nodes, five branches) and as a run's identity (meta save v4, run save v5, the run code's fifth segment); the workshop page; Ore by tier; the tile shop and the Smith's door; the lab at tree states (a base run banks ~22 Ore; the arsenal costs 105); the codex with locked entries and undiscovered fusions; legendary | — | Gate met on the build: finishing a run visibly changes the next one (Ore banked by tier, a win's relic, the workshop's rows); five base runs reach the arsenal by the lab's reading; the codex shows what is still to find. His eye pending |
| 30 | **The Tile Smith in the shell, and the menus** *(Daniil, 2026-09-06 evening: "fold tile smith into the main menu and make it part of the game"; item 30 "the menus need a real graphical rework"; NEXT — the PR list is HANDOVER "Next session, proposed")* | The Tile Smith as a page of the shell opened from the workshop once its door is open, minting into the owned pool with features pricing the tile (§11.1); **the menus reworked to Stone-Story quality** (9.30): frames, ornament, animated titles, the workshop drawn as a tree; the presentation items 9.13, 9.14, 9.19, 9.25, 9.27 through the art brief; the tree sweep with a loadout (tier-2/3 Ore) | Every essential piece of the game is in place; the shell reads as a product (his eye) |
