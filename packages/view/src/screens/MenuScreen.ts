...
export class MenuScreen {
  ...
  render(term: GLTerm, spec: MenuSpec): void {
    ...
    const plateW = Math.min(W - 4, Math.max(34, spec.title.length + 8));
    const x0 = Math.floor((W - plateW) / 2);
    // A solid plate so the board underneath reads as backdrop, not noise.
    ...
    for (const line of spec.body ?? []) {
      term.write(x0 + 1, y++, line.slice(0, plateW - 2), role('ui.dim'));
    }
    ...
    for (const it of spec.items) {
      ...
      term.write(x0 + 2, y, label.padEnd(plateW - 4, ' ').slice(0, plateW - 4), fg, bg);
      if (it.note) term.write(x0 + plateW - 3 - it.note.length, y, it.note, it.disabled ? role('ui.grid') : role('ui.accent'), bg);
      if (!it.disabled) this.regions.push({ row: y, x0: x0 + 2, x1: x0 + plateW - 2, id: it.id });
      y += 2;
    }
    if (spec.footer) term.write(x0 + Math.floor((plateW - spec.footer.length) / 2), y, spec.footer, role('ui.dim'));
  }
